...
public class Jogador implements Serializable {
	...
	// retorna o caractere da posição especÃ­fica da matriz do tabuleiro
	public char getTab(int x, int y){
		return this.tab.tab[x][y];
	}

	// define o caractere em uma posição especÃ­fica da matriz do tabuleiro
	...
	public void posicionarBarco(int x, int y) {
		if (n_barcos < MAX_BARCOS){ // se o número atual de barcos for menor que o máximo
			...
			System.out.println("NÃºmero máximo de barcos excedido.");
		}
	}
	...
	public char atacar(Jogador inimigo, int x, int y){
		...
		} else if (alvo == '~'){ // se o alvo for Ã¡gua
			inimigo.setTab(x, y, '*');
		}
